...
def populate_asset_v1(asset: Asset, result: AssetCreationResult):
    with sub_span("populate_asset_v1"):
...
def populate_asset_v2(asset: Asset, result: AssetCreationResult):
    with sub_span("populate_asset_v2"):
...
def _extract_video_exif_data(asset, result: AssetCreationResult):
    with sub_span("_extract_video_exif_data"):
...
def _extract_image_exif_data(asset: Asset, result: AssetCreationResult):
    with sub_span("_extract_image_exif_data") as span:
        ...
        try:
            ...
            result.invalid = True
            span.set_status(SpanStatus(status_code=StatusCode.ERROR, description="Invalid Image Format"))
            return None
        except FileNotFoundError:
            logger.error("File not found: %s", path)
            result.file_present = False
            span.set_status(SpanStatus(status_code=StatusCode.ERROR, description="File not found"))
            return None
        finally:
