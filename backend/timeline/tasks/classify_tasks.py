...
def init_classify_services(path_to_model):
    with sub_span('init_classify_services'):
...
def run_detector(image_path):
    with sub_span("run_detector") as span:
        span.set_attribute("path", image_path)
...
def object_detection(asset_id):
    with sub_span("[celery] object_detection") as span:
...
def load_and_resize_image(fname, max_dim=1280):
    with sub_span("load_and_resize_image") as span:
        span.set_attribute("path", fname)
...
def image_to_tensor(path):
    with sub_span("image_to_tensor") as span:
