...
def init_face_age_gender():
    with sub_span('init_face_age_gender'):
...
def find_faces2(asset_id, call_match_tasks = True):
    with sub_span("[celery] find_faces2") as span:
        ...
        span.set_attribute("faces", len(face_positions))
        num_faces = 0
        result = []
...
def detect_gender(face_id):
    with sub_span("[celery] detect_gender") as span:
...
def detect_age(face_id):
    with sub_span("[celery] detect_age") as span:
        span.set_attribute("face_id", face_id)
...
def detect_facial_expression(face_id):
    with sub_span("[celery] detect_facial_expression") as span:
        span.set_attribute("face_id", face_id)
...
def find_faces(asset_id, call_match_tasks):
    with sub_span("[celery] find_faces") as span:
        span.set_attribute("asset_id", asset_id)
        asset = Asset.query.get(asset_id)
        if not asset:
            logger.error(
